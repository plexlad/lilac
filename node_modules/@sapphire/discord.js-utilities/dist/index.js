...
__export(src_exports, {
  ...
  isAnyInteraction: () => isAnyInteraction,
...
function isAnyInteraction(messageOrInteraction) {
  return !(messageOrInteraction instanceof import_discord.Message);
}
...
function createPartitionedMessageRow(components) {
  ...
  const messageButtonActionRows = chunkedButtonComponents.map(
    (componentsChunk) => new import_discord2.MessageActionRow().setComponents(componentsChunk)
  );
  const selectMenuActionRows = selectMenus.map(
    (component) => new import_discord2.MessageActionRow().setComponents(component)
  );
...
var _PaginatedMessage = class {
  ...
  addPageEmbeds(embeds) {
    let processedEmbeds = (0, import_utilities5.isFunction)(embeds) ? embeds(
      new import_discord3.MessageEmbed(),
      new import_discord3.MessageEmbed(),
      new import_discord3.MessageEmbed(),
      new import_discord3.MessageEmbed(),
      new import_discord3.MessageEmbed(),
  ...
  addAsyncPageEmbeds(embeds) {
    return this.addPage(async () => {
      let processedEmbeds = (0, import_utilities5.isFunction)(embeds) ? await embeds(
        new import_discord3.MessageEmbed(),
        new import_discord3.MessageEmbed(),
        new import_discord3.MessageEmbed(),
        new import_discord3.MessageEmbed(),
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new import_discord3.InteractionCollector(targetUser.client, {
        ...
        ...!(0, import_utilities5.isNullish)(this.response) && !isAnyInteraction(this.response) ? {
  ...
  async handleCollect(targetUser, channel, interaction) {
    if (interaction.user.id === targetUser.id) {
      ...
      await interaction.reply(
        (0, import_utilities5.isObject)(interactionReplyOptions) ? interactionReplyOptions : { content: interactionReplyOptions, ephemeral: true, allowedMentions: { users: [], roles: [] } }
      );
    }
  }
  handleEnd(_, reason) {
...
var LazyPaginatedMessage = class extends PaginatedMessage {
  ...
  addPageEmbeds(embeds) {
    return this.addPage(() => {
      let processedEmbeds = (0, import_utilities6.isFunction)(embeds) ? embeds(
        new import_discord4.MessageEmbed(),
        new import_discord4.MessageEmbed(),
        new import_discord4.MessageEmbed(),
        new import_discord4.MessageEmbed(),
        new import_discord4.MessageEmbed(),
