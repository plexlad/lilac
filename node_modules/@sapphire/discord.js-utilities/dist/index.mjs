...
import {
  ...
  MessageButton as MessageButton2,
  MessageEmbed,
  MessageSelectMenu as MessageSelectMenu2
} from "discord.js";

// src/lib/PaginatedMessages/utils.ts
import { chunk, partition } from "@sapphire/utilities";
import { Constants, MessageActionRow } from "discord.js";
...
function createPartitionedMessageRow(components) {
  ...
  const messageButtonActionRows = chunkedButtonComponents.map(
    (componentsChunk) => new MessageActionRow().setComponents(componentsChunk)
  );
  const selectMenuActionRows = selectMenus.map(
    (component) => new MessageActionRow().setComponents(component)
  );
    ...
    if (this.pages.length > 1) {
      const messageComponents = await Promise.all(
        [...this.actions.values()].map(async (interaction) => {
          return isMessageButtonInteraction(interaction) ? new MessageButton2(interaction) : new MessageSelectMenu2({
  ...
  setUpCollector(channel, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new InteractionCollector(targetUser.client, {
        ...
        ...!isNullish4(this.response) && !isAnyInteraction(this.response) ? {
...
var LazyPaginatedMessage = class extends PaginatedMessage {
  ...
  addPageEmbeds(embeds) {
    return this.addPage(() => {
      let processedEmbeds = isFunction2(embeds) ? embeds(
        new MessageEmbed2(),
        new MessageEmbed2(),
        new MessageEmbed2(),
        new MessageEmbed2(),
